...
void Editor::BeginDockSpace() {
  ...
  // ImGuiDockNodeFlags_PassthroughCentralNode;
...
void Editor::DrawOverlay(Scene *scene, Camera *camera) {
  ...
  if (corner != -1) {
    ...
    ImVec2 window_pos =
        ImVec2((corner & 1) ? io.DisplaySize.x - DISTANCE : DISTANCE,
               (corner & 2) ? io.DisplaySize.y - DISTANCE
                            : DISTANCE + 20.0f); // +20 for menu bar
  ...
  if (ImGui::Begin("Overlay", nullptr, window_flags)) {
    ...
    if (camera) {
      ...
      float size = 30.0f;
      ImVec2 center = ImVec2(p.x + size + 10, p.y + size + 10);

      glm::mat4 view = camera->GetViewMatrix();
      glm::mat3 viewRot = glm::mat3(view);

      glm::vec3 axes[3] = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
      ImU32 colors[3] = {IM_COL32(255, 50, 50, 255), IM_COL32(50, 255, 50, 255),
                         IM_COL32(80, 80, 255, 255)};
      const char *labels[3] = {"X", "Y", "Z"};

      ImDrawList *draw_list = ImGui::GetWindowDrawList();

      // Draw order sort by Z (View space Z) ?
      // Simple Painter's algo: Sort by Z component of transformed vector?
      // Actually, lines intersecting is hard in 2D draw list.
      // Just draw them.

      for (int i = 0; i < 3; i++) {
        glm::vec3 v = viewRot * axes[i];
        // Y flip for ImGui
        ImVec2 end = ImVec2(center.x + v.x * size, center.y - v.y * size);
        draw_list->AddLine(center, end, colors[i], 3.0f);
        draw_list->AddText(end, colors[i], labels[i]);
      }
...
void Editor::ExecuteCommand(const char *command) {
  ...
  if (cmd == "clear") {
    ClearLogs();
    // Terminali de temizlemesi icin DeveloperConsole'a ilet
...
void Editor::DrawInspector(Scene *scene) {
  ...
  if (meshRenderer && ImGui::CollapsingHeader("Skin / Texture")) {
    ...
    if (meshRenderer->texture) {
      ...
      ImGui::Image((void *)(intptr_t)meshRenderer->texture->GetID(),
                   ImVec2(100, 100));
  ...
  if (weapon && ImGui::CollapsingHeader("Weapon")) {
    const char *weaponTypes[] = {"Rifle",   "Pistol",  "Knife",
    ...
    ImGui::Separator();
    ImGui::Text("Ammo:");
    ImGui::Text("  Magazine: %d / %d", weapon->stats.currentMag,
                weapon->stats.magSize);
...
void Editor::DrawPerformanceMetrics(float deltaTime, float fps) {
  ...
  ImGui::PlotLines("Frame Time (ms)", frameTimes, 90, 0, nullptr, 0.0f, 33.0f,
                   ImVec2(0, 80));
...
void Editor::DrawProjectPanel() {
  ...
  for (auto &directoryEntry :
       std::filesystem::directory_iterator(m_CurrentProjectDir)) {
    const auto &path = directoryEntry.path();
    auto relativePath = std::filesystem::relative(path, m_BaseProjectDir);
    std::string filenameString = path.filename().string();

    if (directoryEntry.is_directory()) {
      if (ImGui::Selectable((filenameString + "/").c_str(), false,
...
void Editor::SpawnEntity(Scene *scene, const std::string &type,
  ...
  if (type == "Cube") {
    ...
    // debug modunda cizdirelim. DebugMesh olmadigi icin simdilik bos
    // birakiyoruz. Ama secilmesi icin Collider lazim olabilir veya
    // Hierarchy'den secilecek. Konumunu gormek icin kucuk bir wireframe
    // cizdirilebilir ama simdilik sadece hierarchy.
  ...
  Log("Spawned " + type + " at " + std::to_string(m_SpawnPosition.x) + ", " +
